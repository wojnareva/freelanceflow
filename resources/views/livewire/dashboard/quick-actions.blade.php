<div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-lg border border-gray-200 dark:border-gray-700">
    <div class="p-6">
        <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-6">{{ __('dashboard.quick_actions.title') }}</h3>
...
{{ __('dashboard.quick_actions.start_timer') }}
            </a>
            ...
            <a href="{{ route('projects.index') }}"
               class="flex items-center justify-center px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200">
                ...
                {{ __('dashboard.quick_actions.new_project') }}
            </a>
            ...
            <a href="{{ route('clients.create') }}"
               class="flex items-center justify-center px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200">
                ...
                {{ __('dashboard.quick_actions.add_client') }}
            </a>
        </div>
        ...
        @if($recentProjects && count($recentProjects) > 0)
            <div class="mb-6">
                <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-3">{{ __('dashboard.project_overview.in_progress') }}</h4>
                <div class="space-y-2">
                    @foreach($recentProjects as $project)
                        <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            ...
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('time-tracking.index') }}?project={{ $project->id }}"
        ...
        @if($recentClients && count($recentClients) > 0)
            <div>
                <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-3">{{ __('dashboard.client_overview.new_clients') }}</h4>
                <div class="space-y-2">
        ...
        @if((!$recentProjects || count($recentProjects) === 0) && (!$recentClients || count($recentClients) === 0))
            <div class="text-center py-8 border-t border-gray-200 dark:border-gray-600">
                ...
                <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('app.get_started') }}</h3>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('dashboard.quick_actions.get_started_text') }}</p>
            </div>
        @endif
    </div>
</div>